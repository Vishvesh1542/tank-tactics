    ...
    async def new_game(self, ctx: bridge.context.BridgeApplicationContext):
        ...
        if isinstance(ctx, bridge.context.BridgeExtContext):
            ...
            current_games.write(key=str(ctx.message.channel.id), value={
                                "creator_id": ctx.message.author.id, 'creator_name': ctx.message.author.name, 'members': [], 'start_time': time.time() + 10, 'started': False})  # 86400 is 24 hours
        # if with / command
        else:
            current_games.write(key=str(ctx.channel_id), value={
                                "creator_id": ctx.author.id, 'creator_name': ctx.author.name, 'members': [], 'start_time': time.time() + 10, 'started': False})
